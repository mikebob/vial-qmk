"""
https://github.com/201dreamers/corne-fw/blob/main/vil_parser.py
...
def rename_keys_according_to_qmk_docs(keymap: VialKeymap) -> VialKeymap:
    ...
    def process_key(key):
        # Recursive function to process keys and their content
        match = re.match(r'LT(\d+)\((.*?)\)', key)
        if match:
            layer_idx, key_content = match.groups()
            if key_content in map_vial_to_qmk:
                key_content = map_vial_to_qmk[key_content]
            return f"LT({layer_idx},{key_content})"
        elif key in map_vial_to_qmk:
            return map_vial_to_qmk[key]
        else:
            # Process content inside functions, e.g., RALT(KC_RBRACKET)
            return re.sub(r'\b(\w+)\((.*?)\)', lambda m: f"{m.group(1)}({process_key(m.group(2))})", key)

    for layout in keymap:
        for row in layout:
            for idx, key in enumerate(row):
                row[idx] = process_key(key)
...
def print_keymap_for_c_code(keymap: VialKeymap) -> None:
    ...
    for layout_index, layout in enumerate(keymap):
        ...
        print(f"[{layout_index}] = LAYOUT_split_3x6_3(\n")
        ...
        for row_index in range(num_of_rows):
            ...
            print(row)
        if layout_index + 1 == len(keymap):  # If it's the last layout, print without a comma
            print(")")
        else:
            print("),")
